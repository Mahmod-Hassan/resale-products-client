...
const ResaleProducts = ({ product, setResaleProduct }) => {
    ...
    const { image, productName, orignialPrice, resalePrice, using, location, sellerName, date, booked } = product;
    return (
        <div className='shadow-xl p-5 rounded'>
            ...
            <p className='text-purple-400 italic'>postedDate : {date}</p>
            ...
            {
                booked === 'booked' ?
                    <button className='btn btn-error text-white disabled'>{booked}</button>
                    :
                    <label
